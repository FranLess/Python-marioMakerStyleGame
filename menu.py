...
class Menu:
        ...
        for key,value in EDITOR_DATA.items():
            if value['menu']:
                if not value['menu'] in self.menu_surfaces:
                    self.menu_surfaces[value['menu']]= [tuple([key,load(value['menu_surf'])])]
                else:
                    self.menu_surfaces[value['menu']].append(tuple([key,load(value['menu_surf'])]))
    ...
    def click(self, mouse_position, mouse_button):
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_position):
                if mouse_button[1]: #middle mouse button
    ...
    def higliht_indicator(self, index):
